...
export default function OrderHistory(){
  ...
  async function search(){
    const res = await fetch((import.meta.env.VITE_API_BASE || 'http://localhost:3001') + '/api/orders/lookup?q=' + encodeURIComponent(q));
  ...
  return (
    <section className="max-w-4xl mx-auto px-4 py-6">
      ...
      <div className="flex gap-2">
        <input className="flex-1 rounded-xl border px-3 py-2" placeholder="Cari dengan Nama/WhatsApp atau link"
          value={q} onChange={e=>setQ(e.target.value)} />
        <button onClick={search} className="px-4 py-2 rounded-xl bg-purple-700 text-white">Cari</button>
      </div>
