...
const resolvedKvUrl =
  resolveEnv("KV_REST_API_URL", "UPSTASH_KV_KV_REST_API_URL", "UPSTASH_KV_REST_API_URL", "UPSTASH_KV_URL") || null;
const resolvedKvToken =
  resolveEnv("KV_REST_API_TOKEN", "UPSTASH_KV_KV_REST_API_TOKEN", "UPSTASH_KV_REST_API_TOKEN", "UPSTASH_KV_TOKEN") || null;
...
async function persistCatalog(list, meta = {}) {
  ...
  if (hasKvEnv && kvClient) {
    try {
      await kvClient.set(CACHE_KEY, JSON.stringify(payload));
    } catch (err) {
      console.warn("[serviceCatalog] kv.set gagal:", err.message);
    }
  }
  try {
...
async function readCachedCatalog() {
  if (hasKvEnv && kvClient) {
    try {
      ...
      console.warn("[serviceCatalog] kv.get gagal:", err.message);
    }
  }
  try {
